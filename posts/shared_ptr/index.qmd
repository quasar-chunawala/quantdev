...
Writing a homegrown version of `shared_ptr<T>` implementation is fun! The destructor is tricky to get right. The libc++ implementation of `shared_ptr<T>` and many others use an atomic reference counter. So, we have something like:
...
One possible implementation of the destructor is the code-snip below.

...
We have the Dupire PDE in terms of the strike. As the strike by itself is not very meaningful, for example, a strike of $K=100$ for a stock worth $S=100$, it's useful to write the Dupire PDE in terms of some scaled value for example, moneyness. 

We define the moneyness as the ratio of the discounted value of the strike, $Ke^{-rT}$ divided by the stock price at time $0$, $S_0$. We can also interprete this as the strike $K$ scaled by the forward price $F$. Remember, that $S_0$ as of today is a fixed number, and its the strike $K$ that differentiates the various call options. Define a new variable $y$ as:

$$
y \stackrel{def}{=} \ln\left(\frac{Ke^{-rT}}{S_0}\right) = \ln\left(\frac{K}{F}\right)
$$

Now, we can easily transform the derivatives in the Dupire PDE in terms of the variable $y$.

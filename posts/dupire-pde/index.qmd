...
Consider the complementary case where local volatility is only a function of strike and is independent of expiration. Let $\Sigma(K)$ be the implied volatility at market level $K$. We have:
...
$$
\begin{align*}
\frac{\partial w(\Sigma, T)}{\partial T} &= \frac{\partial w}{\partial \Sigma} \cdot \frac{\partial \Sigma}{\partial T} + \frac{\partial w}{\partial T} \frac{\partial}{\partial T}(T)\\
&= 2 \Sigma \cdot 0 + \Sigma^2 \\
&= \Sigma^2
\end{align*}
$$

Dropping $\left(\frac{\partial w}{\partial y}\right)^2$ and $\left(\frac{\partial^2 w}{\partial y^2}\right)$ as these are higher order terms, we get:

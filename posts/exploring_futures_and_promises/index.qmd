...
class Task{
    ...
    void precedes(std::vector<Task> tasks)
    {
        for(auto& t : tasks)
            t.m_dependencies.push_back(m_id);
    }

    void succeeds(std::vector<Task> tasks)
    {
        m_dependencies = std::transform(tasks.begin(), tasks.end(),[](Task task){
            return task.m_id;
        });
    }
    ...
    std::vector<u64_t> m_precedes;
    std::vector<u64_t> m_succeeds;
};

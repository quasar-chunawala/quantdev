...
The type metafunction `std::conditional` returns `T`, if the predicate `B` is true, otherwise returns `F`.
...
The type metafunction `std::remove_const` removes the `const` qualifier. It's a *transformation* trait.

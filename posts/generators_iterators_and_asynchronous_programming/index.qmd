...
An iterable and iterator are related concepts in Python but there are subtle differences.

An *iterable* is an object that can be looped over. It has an `__iter__()` method that returns an *iterator*.
...
An iterator is an object that represents a stream of data, as mentioned earlier, that is stateful, meant for one-time use.
...
`for` loops call `iter()` on an iterable to get an iterator, then repeatedly call `next()` on the iterator.

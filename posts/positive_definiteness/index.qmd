...
*Proposition 5*. A twice differentiable function $f:\mathbf{R}^n \to \mathbf{R}$ is convex, if and only if, the hessian $\nabla^2 f(\mathbf{x})$ is positive semi-definite for all $\mathbf{x}\in\mathbf{R}^n$.

*Proof.*

Assume first that $f$ is convex and let $\mathbf{x}\in\mathbf{R}^n$. Define moreover the function $g:\mathbf{R}^n \to \mathbf{R}$ setting:

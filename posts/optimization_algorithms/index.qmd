...
class RMSPropOptimizer:
    ...
    def update_params(self, layer):
        ...
        if not hasattr(layer,'weight_cache'):
            layer.weigh_cache = np.zeros_like(layer.dloss_dweights)
        ...
        layer.bias_cache = self.rho * layer.bias_cache + (1 - self.rho) * layer.dloss_dbiases**2

        # Vanilla SGD parameter update + normalization
        # with square rooted cache
        layer.weights -= self.current_learning_rate * layer.dloss_dweights / (np.sqrt(layer.weight_cache) + self.epsilon)
        layer.biases -= self.current_learning_rate * layer.dloss_dbiases / (np.sqrt(layer.bias_cache) + self.epsilon)

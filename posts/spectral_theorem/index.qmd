...
For $n=1$, any $1\times1$ symmetric matrix is already diagonal.
Since $A$ and $v\in V$ are both scalars, $Av=\lambda v$ where $\lambda=A$.
Thus, we can pick any non-zero scalar $v$ to form a basis of $\mathbf{R}$.
And we can write, $A=P^{-1}\Lambda P$, where $P=I$ and $\Lambda=A$.
...
We know that $C$ is a $n-1\times n-1$ symmetric matrix. By the inductive
hypothesis, $C$ is diagonalizable and further there exists an orthogonal
matrix $R$, such that $R^{-1}CR=D$ where $D$ is diagonal. 
...
Since $R^{T}CR$ is diagonal, it follows that $Q^{-1}AQ$ is diagonal.

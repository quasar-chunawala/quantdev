...
![Internal _Hashtable linked list](_hashtable_02.png){width=75% height=75% fig-align="center"}
...
# Fast and slow hash functions
 ...
 GCC's libstdc++ hash table implementation distinguishes between fast and slow hash functions. If the hash function is slow, then the hash code will be cached inside the hash table node. 

 Currently, `std::hash` is fast by default(including user-defined types), except for `long double` and string-like types (`std::string`, `std::string_view` and all other variations of character types).

 Comparison of two strings with lengths $n$, $m$ has $O(n,m)$ complexity, but if we'll cache the hash value in the hash table node, we can implement faster negative comparison. If hash codes of two strings do not match, we can instantly conclude they are not equal. Integer comparison just takes one CPU cycle.

 For integer types, `std::hash` is defined as the identity function.

 # Insert

 Now, that we know how the hash table is internally organized, let's look at the implementation of `insert`. The high level steps are the following:

 - Check that there is no such key in the hash table.
 - Create a hash table node.
 - Insert the new node to a hash-table data structure.

The implementation is in the `_M_insert_unique` method. And there is a surprise on the first line of code:

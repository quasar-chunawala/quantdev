...
If you apply `decltype` to a name, it will give you the declared type of that name. It's not the same as `auto`, because in `auto` deduction, top-level CV-qualifiers are dropped.
...
Unlike `auto`, `decltype(name)` never strips `const`/`volatile`/references. 

If you have an expression, which is an *lvalue*, `decltype` will figure out its type and will slap an *lvalue reference* on it.

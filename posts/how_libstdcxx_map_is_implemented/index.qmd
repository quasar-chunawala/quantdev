...
The `Hashtable` class itself is a combination of `std::forward_list<_Hash_node>` - a linked list containing the elements belonging to a bucket and `std::vector<std::forward_list<_Hash_node>::iterator>` - a vector(array) of pointers to the buckets.

...
That is not true of the libraries. The linker would very happily agree to link `lib5`, even if it cannot see `foo`. As far as the linker is concerned, the implementation of `foo` might lie in the executable. There is no chance of discovering it. 

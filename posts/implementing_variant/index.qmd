...
The next utility we would like to build is `instance_of`. We want a concept that will tell us, if concrete type is an instance of a certain templated type. For example, `vector<int>` is an instance of `vector<T>`. But, `int` is not an instance of a `vector`. A little bit of a tricky one is : `vector<int>&` is not an instance of `vector`. 
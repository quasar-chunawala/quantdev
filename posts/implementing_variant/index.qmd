...
The `union_` type is something which is used to store the data. It is similar to the built-in union but with some additional functionalities.

For example,

```cpp
constexpr bool union_runtime(){
    dev::tools::union_<int, char, double> x;
    dev::tools::construct_at<2>(x, 1.0);
    if(dev::get<2>(x) != 1.0) return false;
    dev::tools::destroy_at<2>(x);
    dev::tools::construct_at<1>(x, 'a');
    if(dev::get<1>(x) == 'a') return false;
    dev::tools::destroy_at<1>(x);
    return true;
}

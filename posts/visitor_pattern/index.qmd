...
Both implementations are difficult to maintain for large hierarchies. The Visitor pattern offers a solution - it allows us to implement a new operation - in our case, the serialization outside of the classes and without modifying them, but also without the downside of a huge decision tree in a loop. Note that the Visitor pattern is not the obly solution to the serialization probleml C++ offers other possible approaches as well, but we focus on the Visitor pattern.

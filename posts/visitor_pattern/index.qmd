...
struct Pet{
    ...
    virtual void accept(PetVisitor&v, Pet* p = nullptr)
};
```
...
Now, our additional argument is forwarded through the chain of virtual function calls to the visitor, where we can make use of it. Let's create a visitor that records the pet births and adds new pet objects, as children to their parent objects:
 ...
 Note that, if we want to make sure that there are no biological impossibilities in our family tree,the verification has to be done at run-time - at compile-time, we don not know the actual types of the polymorphic objects. The new visitor is just as easy to use as the ones before:

 ```cpp
 Pet* parent(new Cat("orange tabby")); // A cat
 BirthVisitor bv;

 Pet* child(new Cat("calico"));
 parent->visit(bv, child);
 ```

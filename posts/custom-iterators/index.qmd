...
`std::input_iterator` concept models the idea of a read-only iterator. Such an iterator, for example, can be used read packets data from a network socket or a disk file.

Input iterators are also single-pass, but because they are expected to consume a finite number of values they must be comparable to some sentinel value such as `EOF`, `\0` etc.
...
struct SimpleInputIterator {
    ...
    SimpleInputIterator(pointer start)
        : m_file_buffer_ptr{start} {}

    // Dereference operator
    const T& operator*() const {
        return *m_file_buffer_ptr;
    }

...
We first need to show that $M_t \sim \mathcal{N}(0,t)$ or using the characteristic function approach, we need to show that $f_{M_t}(\theta)=\mathbb{E}[e^{\theta M_t}] = e^{\frac{1}{2}\theta^2 t^2}$ for constant $\theta$.

Let $f(t,M_t) = e^{\theta M_t - \frac{1}{2}\theta^2 t}$.
...
\begin{align*}
f(t,M_t) - 1 &= \theta \int_0^t f(s,M_s) dM_s
\end{align*}
...
\begin{align*}
\mathbb{E}[f(t,M_t)] - 1 &= \theta \mathbb{E} \left[ \int_0^t f(s,M_s) dM_s\right]\\
&= 0 \\
\mathbb{E}[f(t,M_t)] &= 1\\
\mathbb{E}[e^{\theta M_t}] &= e^{\frac{1}{2}\theta^2 t}
\end{align*}

which is the moment generating function for the normal distribution with mean $0$ and variance $t$. Therefore, $M_t \sim \mathcal{N}(0,t)$.

Thus, 

$$
\begin{align*}
\mathbb{E}[M_t - M_s] &= \mathbb{E}[\mathbb{E}[M_t - M_s|\mathcal{F}_s]]\\
&= \mathbb{E}[M_s - M_s] = 0
\end{align*}
$$

...
Let's code an `SDE` class and an abstract base class `Solver`.
...
plt.xlabel(r'Time $t$')
plt.ylabel(r'$S(t)$')
plt.title(r'10 sample paths of Geometric Brownian Motion(GBM), $\mu=-0.5$')
plt.grid(True)
for i in range(10):
...
plt.xlabel(r'Time $t$')
plt.ylabel(r'$S(t)$')
plt.title(r'10 sample paths of Geometric Brownian Motion(GBM), $\mu=-2$')
plt.grid(True)
for i in range(10):
    plt.plot(t,solution[i])
...
plt.xlabel(r'Time $t$')
plt.ylabel(r'$S(t)$')
plt.title(r'10 sample paths of Geometric Brownian Motion(GBM), $\mu=0.0$')
plt.grid(True)
for i in range(10):
    plt.plot(t,solution[i])
...
# ornstein-uhlenbeck
ornstein_uhlenbeck_sde = SDE(
    initial_condition=1.0,
    drift = lambda t, s_t : -s_t,
    vol = lambda t, s_t : 1.0,
    dvol_dx = lambda t, s_t : 0.0
)

solution = euler.solve(ornstein_uhlenbeck_sde)
euler.reset()

plt.xlabel(r'Time $t$')
plt.ylabel(r'$S(t)$')
plt.title(r'10 sample paths of Ornstein-Uhlenbeck process')
plt.grid(True)
for i in range(10):
    plt.plot(t,solution[i])
...
diffusion_sde = SDE(
    ...
    drift = lambda t, s_t : np.sqrt(1+s_t**2),
    vol = lambda t, s_t : np.sin(s_t),
    dvol_dx = lambda t, s_t : np.cos(s_t)
)

solution = euler.solve(diffusion_sde)
euler.reset()

plt.xlabel(r'Time $t$')
plt.ylabel(r'$S(t)$')
plt.title(r'10 sample paths of the diffusion')
plt.grid(True)
for i in range(10):
    plt.plot(t,solution[i])
...
**The CIR Model**. Consider the CIR process given by the SDE:

$$
dZ_t = (a - Z_t) dt + \sqrt{Z_t} dB_t, \quad Z_0 = 1
$$ {#eq-cir-process}

(We set the parameters $\sigma=1, b=1$ here.)

(a) Plot $10$ paths of this process on $[0,1]$ with a time step of $0.001$ for each of the cases $a=0.1$
:::
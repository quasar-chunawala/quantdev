...
An important consequence of @th-martingale-representation-theorem is a third definition of Brownian motion.
...
The Martingale representation theorem implies that we can write $M_t = M_0 \int_0^t V_s dB_s$ for some adapted process $(V_t,t \leq T)$. For simplicity, we assume that $V_s$ is such that the process is a martingale. 

We show that the increments of $M$ are independent with the right distribution. In fact, we show that for $0 \leq s < t \leq T$ and $\lambda \in \mathbb{R}$, we have:

$$
\begin{align*}
\mathbb{E}[\exp (\lambda (M_t - M_s))|\mathcal{F}_s] = e^{\frac{\lambda^2}{2}(t-s)}
\end{align*}
$$ {#eq-characteristic-function}

@eq-characteristic-function is sufficient to prove

...
- `H2`, the remaining `7`-bits, used to store metadata for this element. The `H2` bits are stored in the corresponding control byte in the control byte array. 
...
\begin{tikzpicture}[scale=2.5]
  ...
  \node[draw=none, node font=\ttfamily] at (6.53,4.69) {1000 1110};
...
Now, we have a 2-level hash-table. `H1` is `57`-bits, and it tells us where to look in our main array of elements. `H1` is what we take the modulus of, when we are figuring out a slot in the hash-table. `H2` gives us this little bitmask that has to appear in the control bytes. Note, therefore, that the hash function now is extremely important. If the hash-function hides all its entropy in the last `7`-bits, we are going to have a lot of `H1` collisions. If the hash-function puts none of its entropy in the last `7`-bits, we are going to have a lot of `H2` collisions. 
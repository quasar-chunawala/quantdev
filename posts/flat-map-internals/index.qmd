...
# How does `.find()` work for the `raw_hash_set`?

The `find` method branches on `is_small()`, to determine if SOO(Small Object Optimization) is enabled. For the sake of this discussion, assume that we are in the normal mode, SOO is disabled. If that's the case, the control is passed to `find_large(key)`.

```{.cpp include="./raw_hash_set.h" start-line=3258 end-line=3263 code-line-numbers="true" filename="absl/container/internal/raw_hash_set.h"}
```

`find_large(key)` works 

```{.cpp include="./raw_hash_set.h" start-line=3425 end-line=3449 code-line-numbers="true" filename="absl/container/internal/raw_hash_set.h"}
```
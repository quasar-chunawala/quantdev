...
To summarize, therefore, `flat_hash_map` inherits from the base class type deduced as `raw_hash_map<Params...>` where we use the user-supplied policy traits: hash-function, equality and allocator as long as they differ from the default policy.

```{.cpp include="./flat_hash_map.h" start-line=139 end-line=149 code-line-numbers="true" filename="absl/container/flat_hash_map.h"}
```

`raw_hash_map` type itself inherits from `raw_hash_set`.

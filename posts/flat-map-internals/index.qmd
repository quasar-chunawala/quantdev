...
```{.cpp include="./raw_hash_set.h" start-line=965 end-line=966 code-line-numbers="true" filename="absl/container/internal/raw_hash_set.h"}
```
...
```{.cpp include="./raw_hash_set.h" start-line=978 end-line=984 code-line-numbers="true" filename="absl/container/internal/raw_hash_set.h"}
```
...
The control bytes array containing metadata and the main array of elements are split into `Group`s of size `Group::kWidth` elements each. `Group::kWidth` is a magic number set to `16` elements. 
...
```{.cpp include="./hashtable_control_bytes.h" start-line=505 end-line=505 code-line-numbers="true" filename="absl/container/internal/hashtable_control_bytes.h"}
```
...
```{.cpp include="./raw_hash_set.h" start-line=1802 end-line=1827 code-line-numbers="true" filename="absl/container/internal/raw_hash_set.h"}
```
...
```{.cpp include="./hashtable_control_bytes.h" start-line=290 end-line=293 code-line-numbers="true" filename="absl/container/internal/hashtable_control_bytes.h"}
```
...
```
[Compiler Explorer](https://godbolt.org/z/8PMfq8oj9)
...
`_mm_movemask_epi8` takes our `128`-bit thing and squishes it. It takes the high-bit of each byte and gives us a `16`-bit thing, that has `0`s where they didnt match and `1`s where they do match. This tell us which of our `16`-control bytes had the proper `h2(hash)`. 
...
```
[Compiler Explorer](https://godbolt.org/z/Mb9a4853T)
...
```{.cpp include="./hashtable_control_bytes.h" start-line=276 end-line=287 code-line-numbers="true" filename="absl/container/internal/hashtable_control_bytes.h"}
```
...
```{.cpp include="./hashtable_control_bytes.h" start-line=123 end-line=123 code-line-numbers="true" filename="absl/container/internal/hashtable_control_bytes.h"}
```
...
```{.cpp include="./hashtable_control_bytes.h" start-line=81 end-line=108 code-line-numbers="true" filename="absl/container/internal/hashtable_control_bytes.h"}
```
...
```{.cpp include="./hashtable_control_bytes.h" start-line=89 end-line=91 code-line-numbers="true" filename="absl/container/internal/hashtable_control_bytes.h"}
```
...
```{.cpp include="./hashtable_control_bytes.h" start-line=61 end-line=65 code-line-numbers="true" filename="absl/container/internal/hashtable_control_bytes.h"}
```

The `countr_zero(T x)` is utility function defined in the `absl/numeric/bits.h` header.


```{.cpp include="./bits.h" start-line=117 end-line=121 code-line-numbers="true" filename="absl/numeric/bits.h"}
```
...
```{.cpp include="./numeric/internal/bits.h" start-line=351 end-line=365 code-line-numbers="true" filename="absl/numeric/internal/bits.h"}
```

This function first runs a bunch of compile-time `assert`s. `T` must be an unsigned type. [std::numeric_limits<T>::digits](https://en.cppreference.com/cpp/types/numeric_limits/digits) is the number of digits in std::numeric_limits<T>::radix base(`2` for fundamental types) that the type `T` can hold. So, for example, `std::numeric_limits<uint32_t>::digits` equals `32`. The function checks at compile-time if the number of digits that can be held by `T` is a power of `2`. 
...
```{.cpp include="./numeric/internal/bits.h" start-line=284 end-line=304 code-line-numbers="true" filename="absl/numeric/internal/bits.h"}
```
...
```
[Compiler Explorer](https://godbolt.org/z/ErMa7dxxT)

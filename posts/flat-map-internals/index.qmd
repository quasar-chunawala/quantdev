...
`NonIterableBitMask` represents an abstract bitmask, such as one emitted by a SIMD instruction. The type `T` controls the storage used for the bitset. For example, if `T = uint32_t`, the bitset is stored in `32` (real) bits of memory. `SignificantBits` represents the the number of abstract bits in the bitset.  
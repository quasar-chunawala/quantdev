...
[Compiler Explorer](https://godbolt.org/z/TTfKs4seP)
...
So, this is going to elide the copy. 

The C++ compiler can perform a similar thing when passing arguments by value.
...
This is how it actually works. We are going to create our temporary - the string `"A"` in the place, where we would have actually copied it, that is, the local variable `a` in the stack frame of `f(std::string)`. 

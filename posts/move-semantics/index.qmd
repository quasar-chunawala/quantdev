...
*Step 3*. When `push_back(const T&)` returns, the temporary `x + x` will die and its destructor is called and the memory allocated on the heap is freed. You can see this on [cppinsights](https://cppinsights.io/s/3f814c34){target="_blank"}.
...
Alright, so this is what's going on. Our function `main()` is going to call `discountFactor(double, double)` in order to populate a local `df`. The stack frame for 
...
```cpp
Vector& operator=(Vector other) //(1)
{

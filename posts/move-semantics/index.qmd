...
void callFoo(T&& arg)
{
    foo(std::forward<T>(arg));  // becomes foo()
}
```

Like `std::move()`, `std::forward()` is also defined as a function template in the header file `<utility>`. The expression `std::forward<T>(arg)` is essentially implemented as follows:
...
Just like for `std::move()`, the semantic meaning of `std::forward()` is *I no longer need this value here*, with the additional benefit that we can preserve the type (including `const`-ness) and the *value category* of the object the passed universal reference points to. 
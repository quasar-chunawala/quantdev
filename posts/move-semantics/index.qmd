---
title: "A hitchhiker's guide to Move Semantics"
author: "Quasar"
date: "2024-10-26"
categories: [C++]      
image: "cpp.jpg"
...
Hey, this is cool! Why don't we apply this idea to the below example?
...
`std::move()` is a function that unconditionally casts its argument to an *rvalue*. `std::move(v1)` marks `v1` to be movable. It does not physically move anything. It signals, that the object `v1` may be *moved from*.  

It allows the implementation of the call to benefit from this mark, by performing some optimization when copying the value, such as stealing the memory. Whether the memory/resource is stolen, is something the caller does not know.

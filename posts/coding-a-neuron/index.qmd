...
Consider what happens when the input is $-1.00$. The output of the first neuron is $1.50$ which implies that the second neuron is deactivated, so the output of the second neuron is simply zero. 
...
Let's now take this concept and use it to fit to the sine way function using two hidden layers of $8$ neurons each and we can hand-tune the values to fit the curve. We'll do this by working with $1$ pair of neurons at a time, which means $1$ neuron from each layer individually. 
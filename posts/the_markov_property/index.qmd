...
Let's say that we are interested in the motion along the horizontal $x$-axis. Let's say we drop brownian particles in a liquid. Let $f(t,x)$ represent the number of particles per unit volume (density) at position $x$ at time $t$. So, the number of particles in a small interval of width $dx$ will be $f(t,x)dx$. 

Now, as time progresses, the number of particles in this region will change. Because, the brownian particles will zig-zag upon bombardment by the molecules of the liquid. Some particles will move out of the region, while other particles will move in. 

Let's consider a timestep of length $\tau$. Einstein's probabilistic approach was to model the distance travelled by the particles or displacement of the particles as a random variable $\Delta$. To determine how many particles end up in the region $R$, we start with the area to the right of the region $R$.

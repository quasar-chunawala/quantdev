...
We are going to derive the diffusion equation now, and we are going to follow Einstein's approach that he came up with 1905. 

Let's say that we are interested in the motion along the horizontal $x$-axis. Let's say we drop brownian particles in a liquid. Let $f(t,x)$ represent the number of particles per unit volume (density) at position $x$ at time $t$. So, the number of particles in a small interval $I=[x,x+dx]$ of width $dx$ will be $f(t,x)dx$. 
...
Now, if we integrate these movements across the real line, then we get the number of particles at $x$ at a later time $t + \tau$. 
...
We are going to solve the diffusion equation, so we are going to determine the function $f(t,x)$ subject to some initial conditions. 

We are going to use the *similarity solutions method*. The method is very simple and very commonly used. But, usually the details are hidden. So, when you hear things like, *let's consider the following transformations*, and those transformations somehow turn out to be the magic transformations, you know that the similarity method has been used, most likely as a software package. We are not going to discuss the general similarity solutions method. We are going to stay glued to the diffusion equation. We are going to derive the equations heuristically using our understanding of the physical processes. And you will get the gist of the similarity solutions method under familiar circumstances. 

Essentially, the approach is all about:

- Find the invariant transformation
- Using those transformations to reduce the number of variables in a PDE
- Solve the simpler equation

In our case, we have two variables $t$ and $x$, so we are hoping this method will help us reduce it to one variable. 

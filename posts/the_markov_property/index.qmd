...
The solution of the PDE can be written in the form:

$$
f(t,x) = \mathbb{E}[g(B_t)|B_0 = x]
$$ {#eq-solution-to-the-heat-equation}
...
for some given function $g$ of space. The form of the solution (@eq-solution-to-the-heat-equation) has a natural interpretation. The function $g(x)$ can be seen as representing the temperature at position $x$ at time $0$. The value $f(t,x)$ gives the temperature at time $t$ and position $x$. In other words, $f(t,x)$ is a specific type of space average. Effectively, it is a smoothing of the initial function $g$ that models the diffusion of heat. 

Note that since the PDF of $B_t$ given $B_0 = x$ is given by the heat kernel $p_t(x,y)$ in @eq-brownian-motion-transition-density-function, the solution of the initial value problem of the heat equation can be written as:

$$
f(t,x) = \int_{-\infty}^{\infty}g(y)p_t(x,y)dy
$$ {#eq-solution-to-the-heat-equation-as-integral}
:::

The heat equation is a particular instance of the *Kolmogorov backward equation*, or simply the *backward equation*. The solution of these PDEs can be represented as an average over paths of diffusions, similar to the representation (@eq-solution-to-the-heat-equation). To prove this, we will be heavily using the fact that the local volatility and drift of diffusions are functions of the diffusion's position and time only.

::: {#thm-backward-equation-with-initial-value}

### Backward equation with initial value

Let $(X_t,t\geq 0)$ be a diffusion in $\mathbb{R}$ with SDE:
...
$$
\begin{align*}
\frac{\partial f}{\partial t}(t,x) &= \frac{\sigma(x)^2}{2}\frac{\partial^2 f}{\partial x^2}(t,x) + \mu(x)\frac{\partial f}{\partial x}(t,x)\\
f(0,x) &= g(x)
\end{align*}
$$
...
Let's fix $t$ and consider the function of space $h(x) = f(t,x) = \mathbb{E}[g(X_t)|X_0 = x]$. Ito's formula applied to $h$ yields:

$$
\begin{align*}
dh(X_s) &= h'(X_s) dX_s + \frac{1}{2}h''(X_s) dX_s dX_s\\
&= \frac{\partial f}{\partial x}(s,X_s) (\sigma(X_s)dB_s + \mu(X_s)ds) + \frac{1}{2}\frac{\partial^2 f}{\partial x^2}(s,X_s)(\sigma(X_s)^2) ds
\end{align*}
$$

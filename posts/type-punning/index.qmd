...
We have a type - we have a `struct X` and then you have some memory - some bytes, which come from somewhere, in this case, I am just allocating them with `malloc` and then you cast them to an `X`. I guess, I now have a pointer to an `X` now, which is great. I can set the members of `X` and return that object.

However, in C++, the above code is UB.

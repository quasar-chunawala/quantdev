...
Suppose we are looking at the bit representation of an `float` and want to pretend, it's an `int`, the correct way to do this in C++ is `std::bit_cast`. I allude to this discussion at length further ahead.
...
> [[basic.lval]/11.3](https://eel.is/c++draft/basic.lval#11) An object of dynamic type $T_{obj}$ is *type-accessible* through a reference of $T_{ref}$ if $T_{ref}$ is similar to:
>
> - the dynamic type of the object.
> - a type that is signed or unsigned type corresponding to the dynamic type of the object.
> - a `char`, `unsigned char` or `std::byte` type.
>
> If a program attempts to access the stored value of an object through a reference that is not type-accessible, the behavior is undefined.
...
> [[basic.stc.static]/1](https://eel.is/c++draft/basic.stc#static) All variables which belong to a **namespace scope** or are first declared with the `static` or `extern` keywords have **static storage duration**. The storage for these entities lasts for the duration of the program.
...
int* ptr = ::new(static_cast<void*>(raw_mem)) int(42);  
```

The C++ standard library also provides a `constexpr` wrapper over placement `new`- `std::construct_at`.
...
We can also use the standalone function `std::destroy_at`.
...
std::free(raw_mem);     // deallocate storage
```

Here, we have the object lifecycle completely explicit. 
...
More interesting is the array of `unsigned char` or `std::byte`. 
...
```
[Compiler Explorer](https://compiler-explorer.com/z/c6vhzfnc8)

We can also do something more interesting. We can reuse the memory of an existing object.
...
```
[Compiler Explorer](https://godbolt.org/z/YdobYbKKv)
...
> [[class.prop]/9] A class `S` is an implicit-lifetime class if:
> 
...
Anything that starts the lifetime of an `unsigned char` or `std::byte` array also implictly creates an `int`. 
...
`std::memcpy` and `std::memmove` implicitly create an objects, if their type is an implicit lifetime type.
...
### How does the compiler know what objects to create?
...
Consider the canonical example on [cppreference.com](https://en.cppreference.com/cpp/memory/start_lifetime_as). `[1]` is UB, because it violates strict-aliasing and object lifetime rules.
`[2]` is also UB, because `std::launder` can't create an object where none exists in the first place. `std::start_lifetime_as<T>` can be used to explicitly trigger the creation and start the lifetime of an object whose type is an implicit lifetime type.
...
Remember, this does not call `complex<float>` constructor - it just declares - *the bytes here are now to be understood as `complex<float>`*.
...
> [[basic.align]/6.8.3](https://eel.is/c++draft/basic.align) Object types have *alignment requirements* which place restrictions on the addresses at which an object of the that type may be allocated. An *alignment* is an implementation-defined integer value representing the number of bytes between successive addresses at which a given object can be allocated. 
>
> An object type imposes alignment requirements on every object of that type; stricter alignment can be requested using the `alignas` specifier.
...
Suppose we are looking at the bit representation of an `float` and want to pretend, it's an `int`, the correct way to do this in C++ is `std::bit_cast`. `std::bit_cast` is a `constexpr` wrapper over `std::memcpy`. `memcpy` is the fastest library routine for memory-to-memory copy. The compiler intrinsic `__builtin_memcpy` compiles down to a single machine instruction `load_aligned`, or `load_unaligned` in case the source is unaligned.

`std::memcpy` doesn't work for all types - there are some restrictions. Both the types `From` and `To` must be trivially copyable. 

`std::bit_cast` is a drop-in replacement for `reinterpret_cast`. 
...
- [A deep dive into C++ object lifetimes](https://www.youtube.com/watch?v=oZyhq4D-QL4&t=618s) by Jonathan Muller, C++ Now 2024.
- [Type punning in modern C++](https://www.youtube.com/watch?v=_qzMpk-22cc) by Timur Doumer, CppCon 2019.

...
According to the standard, implicit object creation works for implicit lifetime types.
...
`std::memcpy` and `std::memmove` implicitly create an object.
...
`[2]` is also UB, because `std::launder` can't create an object where none exists in the first place.

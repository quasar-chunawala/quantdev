...
A semaphore can potentially act as a mutex if the number of permits it can give is set to $1$. However, the most important difference is that, the thread that calls `acquire()` on a mutex must subsequently `release()` the mutex. A mutex is *owned* by the thread acquiring it, upto the point the owning thread releases it. Whilst, in the case of a binary semaphore, different threads can call `acquire()` and `release()` on the semaphore. 
